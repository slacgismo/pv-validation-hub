...
class InsertAnalysis:
    ...
    def buildSystemMetadata(self):
        ...
        overlapping_systems = pd.merge(
            self.new_sys_metadata_df[["name", "latitude", "longitude"]],
        ...
        self.new_sys_metadata_df = self.new_sys_metadata_df[
            ~self.new_sys_metadata_df["name"].isin(list(overlapping_systems["name"]))
        ]
    ...
    def buildFileMetadata(self, s3_path):
        ...
        file_ids = [*range(max_file_id, max_file_id + len(self.new_file_metadata_df))]
        self.new_file_metadata_df["file_id"] = file_ids
        # Build out the S3 path for each file name
        self.new_file_metadata_df["base_file_name"] = self.new_file_metadata_df[
            "file_name"
        ]
        self.new_file_metadata_df["file_name"] = [
            os.path.join(s3_path, x)
            for x in list(self.new_file_metadata_df["file_name"])
        ]
    ...
    def insertConfig(self, evaluation_folder_path):
        ...
        shutil.copyfile(self.config_file_path, os.path.join(new_folder, "config.json"))
    ...
    def generateFileTestLinker(self, new_evaluation_folder):
        ...
        # Write to the folder
        file_test_link.to_csv(os.path.join(new_evaluation_folder, "file_test_link.csv"))
        return
