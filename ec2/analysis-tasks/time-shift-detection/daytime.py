...
def _rolling_by_minute(data, days, f):
    ...
    # the day.
    rolling = data.groupby(data.index.hour * 60 + data.index.minute).rolling(
        min_periods=1, center=True, window=days
    )
...
def _correct_if_invalid(series, invalid, correction_window):
    ...
    rolling_majority = _rolling_by_minute(
        series, days=correction_window, f=lambda x: x.sum() / x.count() > 0.5
    )
    return (~invalid & series) | (invalid & rolling_majority)


def _correct_midday_errors(
...
def _correct_edge_of_day_errors(
    night,
    minutes_per_value,
    ...
    day_length = (
        1 + day_periods.groupby(night.cumsum()).transform("sum")
    ) * minutes_per_value
...
def _ffill_short_periods(night, minutes_per_value, hours_min):
    ...
    invalid.loc[
        invalid.index.date <= (invalid.index.date.min() + pd.Timedelta(days=1))
    ] = False
    invalid.loc[
        invalid.index.date >= (invalid.index.date.max() - pd.Timedelta(days=1))
    ] = False
...
def power_or_irradiance(
    ...
    rolling_median = _rolling_by_minute(
        series_norm, days=median_days, f=pd.core.window.RollingGroupby.median
    )
    ...
    night_corrected_clipping = ~(
        (clipping or False) | (~night_corrected_midday)
    )
...
def get_sunrise(daytime_mask, freq=None, data_alignment="L"):
    ...
    sunrise_series = (
        daytime_mask.index[daytime_mask]
        .to_series()
        .groupby(daytime_mask[daytime_mask].index.date)
        .transform("first")
        .reindex(daytime_mask.index)
    )
    sunrise_series = (
        sunrise_series.groupby(sunrise_series.index.date).ffill().bfill()
    )
    # Backfilling and front filling fills all NaN's, so we set cases not in
    # the right day to NaN
    sunrise_series.loc[sunrise_series.index.date != sunrise_series.dt.date] = (
        np.nan
    )
    ...
    # from each sunrise time in the sunrise_series.
        ...
        raise ValueError(
            "No valid data alignment given. Please pass 'L'"
...
def get_sunset(daytime_mask, freq=None, data_alignment="L"):
    ...
    sunset_series = (
        sunset_series.groupby(sunset_series.index.date).ffill().bfill()
    )
    # Backfilling and front filling fills all NaN's, so we set cases not in
    # the right day to NaN
    sunset_series.loc[sunset_series.index.date != sunset_series.dt.date] = (
        np.nan
    )
    # If there's no frequency value, infer it from the daytime_mask series
    if not freq:
