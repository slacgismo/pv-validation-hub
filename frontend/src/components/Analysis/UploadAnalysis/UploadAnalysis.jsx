...
export default function UploadAnalysis(props) {
  ...
  const handleSubmit = () => {
    const response = AnalysisService.uploadAnalysis(
      props.user_id,
  ...
  return (
    <Container>
      <Alert sx={{ display: errored }} severity="error">
        Failed to create analysis -
        {analysisFormValues.name}
      </Alert>
      <Alert sx={{ display: success }} severity="success">
        Successfully created analysis -
        {analysisFormValues.names}
      </Alert>
      <Container sx={{
        marginTop: 5,
        marginLeft: 30,
