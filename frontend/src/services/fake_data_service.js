...
export function create_fake_submission_row(ranking_number) {
    let analysis_id = faker.datatype.number();
    let ranking_id = faker.datatype.number();
    let developer_name = faker.name.firstName();
    let score = faker.datatype.number();
    let submit_time = faker.helpers.arrayElement([
        faker.datatype.number({ 'min': 1, 'max': 23 }) + 'h',
        faker.datatype.number({ 'min': 1, 'max': 30 }) + 'd'
    ]);
    let execution_time = faker.helpers.arrayElement([
        faker.datatype.number({ 'min': 1, 'max': 23 }) + 'h',
        faker.datatype.number({ 'min': 1, 'max': 30 }) + 'd'
    ]);
    let error = <Tooltip title="Error-Failed"><ErrorIcon /></Tooltip>;
    let algorithm_url = <a href={faker.internet.url()}><LaunchIcon /></a>;
    return {
        analysis_id,
        ranking_id,
        developer_name,
...
export function create_fake_leaderboard_row(ranking_number) {
    let created_by = faker.name.fullName();
    let score = faker.datatype.number();
    let algorithm = <a href={faker.internet.url()}><LaunchIcon /></a>;
...
export function fake_reply_array(number, count, replyingTo) {
    ...
    for (let i = 0; i < number; i++) {
        let temp = create_fake_reply_comment(temp_count);
...
export function fake_comments_array(number) {
    ...
    var count = 1;
    for (let val = 0; val < number; val++) {
        var temp = create_fake_comment(count);
        if (val % 4 === 0) {
